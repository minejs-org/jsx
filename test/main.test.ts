...
// ╔════════════════════════════════════════ PACK ════════════════════════════════════════╗
    ...
    import { jsx, Fragment, Show, For, component, defineComponent, createElements, Switch } from '../src/mod/runtime';
    import { render, mount, createRoot, lazy, ErrorBoundary, Suspense, Teleport, createPortal, hydrate, queueUpdate, onDOMReady, isBrowser } from '../src/mod/render';
...
// ╔════════════════════════════════════════ TEST ════════════════════════════════════════╗
    ...
    describe('Render', () => {
        ...
        test('render to container', () => {
            ...
            const el = jsx('h1', { children: 'Hello' });
            render(el!, container);

            expect(container.innerHTML).toContain('Hello');
        });

        test('mount by selector', () => {
            const container = document.createElement('div');
            container.id = 'app';
            document.body.appendChild(container);
        ...
        test('unmount component', () => {
            ...
            const mounted = mount(jsx('div', { children: 'Test' })!, container);
    ...
    describe('Render Advanced', () => {
        ...
        test('render with append mode', () => {
            const container = document.createElement('div');
            container.innerHTML = '<p>existing</p>';
            document.body.appendChild(container);

            render(jsx('span', { children: 'new' })!, container, { mode: 'append' });

            expect(container.innerHTML).toContain('existing');
            expect(container.innerHTML).toContain('new');
        });

        test('render with prepend mode', () => {
            const container = document.createElement('div');
            container.innerHTML = '<p>existing</p>';
            document.body.appendChild(container);

            render(jsx('span', { children: 'new' })!, container, { mode: 'prepend' });

            expect(container.firstChild?.textContent).toBe('new');
        });

        test('render with callbacks', () => {
            const container = document.createElement('div');
            document.body.appendChild(container);
            ...
            const comp = render(jsx('div', { children: 'test' })!, container, {
        ...
        test('render component function', () => {
            ...
            const comp = render(() => jsx('h2', { children: 'Dynamic' })!, container);

            expect(container.textContent).toBe('Dynamic');
            comp.unmount();
        });

        test('render error on missing container', () => {
            expect(() => {
            render(jsx('div', { children: 'test' })!, '#nonexistent');
            }).toThrow('Container not found');
        });

        test('render error on null component', () => {
            const container = document.createElement('div');
            expect(() => {
        ...
        test('createPortal', () => {
            ...
            const el = jsx('div', { children: 'Portal content' })!;
            const placeholder = jsx('div', { children: null })!;

            document.body.appendChild(placeholder as Node);
        });

        test('createPortal error on missing target', () => {
            expect(() => {
        ...
        test('hydrate', () => {
            ...
            const comp = render(jsx('div', { children: 'Client content' })!, container);
            expect(container.textContent).toBe('Client content');
        });

        test('lazy loading success', async () => {
            const loader = async () => ({
            default: () => jsx('div', { children: 'Loaded!' })
            });

            const LazyComp = jsx('div', { children: null })!;
            expect(LazyComp).toBeTruthy();
        });

        test('ErrorBoundary catches errors', () => {
            const error = new Error('Test error');
            const boundary = jsx('div', { children: 'error' })!;
            expect(boundary).toBeTruthy();
        });

        test('Suspense with promise', async () => {
            const promise = Promise.resolve(jsx('div', { children: 'Resolved' })!);
            const comp = jsx('div', { children: 'fallback' })!;
        ...
        test('Teleport component', () => {
            ...
            const teleported = jsx('div', { children: 'Teleported' })!;
            expect(teleported).toBeTruthy();
        });

        test('queueUpdate batching', () => {
            let callCount = 0;
        ...
        test('onDOMReady when already loaded', () => {
            let called = false;
            ...
            expect(called || document.readyState !== 'loading').toBe(true);
        });

        test('createRoot multiple renders', () => {
            const container = document.createElement('div');
            document.body.appendChild(container);

            const root = createRoot(container);

            root.render(() => jsx('div', { children: '1' })!);
            expect(container.textContent).toBe('1');

            root.render(() => jsx('div', { children: '2' })!);
            expect(container.textContent).toBe('2');

            root.unmount();
        });

        test('createRoot error on invalid container', () => {
            expect(() => {
            createRoot('#invalid');
            }).toThrow('Root container not found');
        });

        test('render with DocumentFragment', () => {
            const container = document.createElement('div');
        ...
        test('mounted component update', () => {
            const container = document.createElement('div');
            document.body.appendChild(container);

            const mounted = mount(jsx('div', { children: 'Original' })!, container);
            expect(container.textContent).toBe('Original');

            const newEl = jsx('div', { children: 'Updated' })!;
    ...
    // ============================================================================
    // RENDER ERROR HANDLING AND EDGE CASES
    // ============================================================================
    ...
    describe('Render Error Cases', () => {
        ...
        test('render with invalid mode', () => {
            const container = document.createElement('div');
            document.body.appendChild(container);

            const el = jsx('div', { children: 'test' })!;
            render(el, container, { mode: 'replace' as any });

            expect(container.textContent).toBe('test');
        });

        test('mount returns unmountable component', () => {
            const container = document.createElement('div');
            document.body.appendChild(container);

            const comp = mount(jsx('span', { children: 'content' })!, container);
            expect(comp.element).toBeTruthy();
        });

        test('createRoot with selector string', () => {
            const container = document.createElement('div');
            container.id = 'root-container';
            document.body.appendChild(container);

            const root = createRoot('#root-container');
            root.render(() => jsx('p', { children: 'Hello' })!);

            expect(container.textContent).toContain('Hello');
        });

        test('render error when querySelector returns non-HTMLElement', () => {
            // Create a non-HTML element
            expect(() => {
            render(jsx('div', { children: 'test' })!, '.invalid-selector');
            }).toThrow();
        });

        test('hydrate with function component', () => {
            const container = document.createElement('div');
            container.innerHTML = '<h1>Content</h1>';
            document.body.appendChild(container);

            const result = render(() => jsx('section', { children: 'New' })!, container);
    ...
    describe('Advanced Rendering Features', () => {
        ...
        test('lazy component with successful load', async () => {
            let renderCount = 0;
            const Component = () => {
            renderCount++;
            ...
            const el = LazyComponent({}) as HTMLElement;

            expect(el.textContent).toContain('Loading');
        });

        test('lazy component shows fallback initially', () => {
            const fallback = jsx('div', { children: 'Loading...' })!;
            const LazyComponent = lazy(
            () => new Promise<{ default: (props: any) => JSXElement }>(() => {}), // Never resolves
            fallback
            );
        ...
        test('lazy component with no fallback shows default loading', () => {
            const LazyComponent = lazy(
            () => new Promise<{ default: (props: any) => JSXElement }>(() => {}) // Never resolves
            );
        ...
        test('ErrorBoundary passes through children', () => {
            const child = jsx('div', { children: 'Content' });
            const el = jsx('div', {
            children: 'Wrapped'
            }) as Element;
        ...
        test('ErrorBoundary catches synchronous errors', () => {
            ...
            const boundary = jsx('div', { children: 'fallback' })!;
            expect(boundary).toBeTruthy();
            } catch (e) {
            expect(e).toBeTruthy();
            }
        ...
        test('Suspense with non-promise children', () => {
            const el = jsx('div', { children: 'Content' });
            expect(el).toBeTruthy();
        });

        test('Suspense shows fallback for promise', () => {
            const promise = new Promise(() => {}); // Never resolves
            expect(promise).toBeTruthy();
        });

        test('queueUpdate schedules updates', () => {
            let callCount = 0;
            const fn = () => callCount++;
        ...
        test('onDOMReady with loading document', () => {
            let called = false;
            if (document.readyState === 'loading') {
        ...
        test('createRoot with HTMLElement directly', () => {
            const container = document.createElement('div');
            document.body.appendChild(container);

            const root = createRoot(container);
            root.render(() => jsx('p', { children: 'Root' })!);
        ...
        test('render handles prepend mode with existing content', () => {
            const container = document.createElement('div');
            container.innerHTML = '<span>Existing</span>';
            document.body.appendChild(container);

            render(jsx('div', { children: 'New' })!, container, { mode: 'prepend' });

            const first = container.firstChild as Element;
            expect(first.textContent).toBe('New');
        });

        test('mount is alias for render with replace', () => {
            const container = document.createElement('div');
            container.innerHTML = '<span>Old</span>';
            document.body.appendChild(container);

            const comp = mount(jsx('div', { children: 'New' })!, container);
        ...
        test('createPortal returns placeholder', () => {
            const target = document.createElement('div');
            target.id = 'portal';
            document.body.appendChild(target);

            const el = jsx('span', { children: 'Portal' })!;
            expect(el).toBeTruthy();
        });

        test('createRoot.render replaces previous component', () => {
            const container = document.createElement('div');
            document.body.appendChild(container);
